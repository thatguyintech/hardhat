...
Either the archival node provider you are using is down, or it's very possible that your internet connection is blocking the connection. For example, your VPN or your internet provider is limiting your connection to the archival node provider. One way to fix this is to try connecting to a different internet network.
